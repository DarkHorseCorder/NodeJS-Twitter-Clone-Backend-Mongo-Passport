...
timelineSchema.statics.getTimeline = async function (
   ...
   */
  if (!user_id) {
    let { _id } = await mongoose.model("User").findOne({ screen_name }, "_id");
    user_id = _id;
  }
  let { posts } = await mongoose
    .model("home_timeline") //or this
    .findOne({ user_id }, { posts: { $slice: [s * (p - 1), s] } }) //returns
    .populate({
...
timelineSchema.methods.bulkRemovePosts = async function ({
  ...
  } else if (id_post_removed) {
    // remove this post only
    posts_to_remove = await mongoose.model("Post").find(
      {
        _id: id_post_removed,
      },
